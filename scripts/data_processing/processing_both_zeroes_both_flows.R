...
remove(night_flag,night_zero_avg,night_zeroes_grouped,night_zeroing)
...
hono_hourly_za = hono_za %>% 
  timeAverage("1 hour")


# Comparing night and za zeroing ------------------------------------------

hono_hourly_night %>% 
  left_join(hono_hourly_za,by = "date") %>% 
  pivot_longer(c(hono_night,hono_night_constant,hono_za,hono_za_constant)) %>% 
  ggplot(aes(date,value,col = name)) +
  theme_bw() +
  geom_path(size = 1) +
  theme(legend.position = "top",
...
# ggsave('hono_different_zeroing.png',
#        path = "output/analysis_plots",
#        width = 30,
#        height = 12,
#        units = 'cm')
...
dat = hono_night %>% 
  left_join(hono_za,by = "date") %>% 
  mutate(local_pollution = ifelse(date >= "2024-09-11" & date < "2024-09-13","polluted","clean"))

# write.csv(dat,"output/processed_data/processed_hono_night_za_zeroes.csv",row.names = F)

diurnal_dat = dat %>% 
  mutate(hour = hour(date),
         local_pollution = ifelse(date >= "2024-09-11" & date <= "2024-09-13 13:00","polluted","clean")) %>% 
  filter(local_pollution == "clean") %>% 
  group_by(hour) %>% 
  select(-date) %>%
...
diurnal_dat %>% 
  pivot_longer(c(hono_night_constant_mean,hono_night_mean,hono_za_constant_mean,hono_za_mean)) %>% 
...
# ggsave('hono_different_zeroing_diurnal.png',
#        path = "output/analysis_plots",
#        width = 30,
#        height = 12,
#        units = 'cm')
